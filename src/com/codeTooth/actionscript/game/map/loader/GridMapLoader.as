package com.codeTooth.actionscript.game.map.loader
{
	...
	public class GridMapLoader extends Sprite implements IDestroy
	{
		private var _debug:Boolean = false;
		
		public function GridMapLoader(mapWidth:uint, mapHeight:uint, 
									  ...
									  canvasTransparent:Boolean = false, canvasColor:uint = 0x00000000, 
									  mergeAlpha:Boolean = false, bmpdAreaWidth:uint = 2000, bmpdAreaHeight:uint = 2000)
		{
			if(cellWidth == 0)
			{
			...
			_canvasTransparent = canvasTransparent;
			...
			_bmpdAreaWidth = bmpdAreaWidth;
			_bmpdAreaHeight = bmpdAreaHeight;
		...
		private var _canvas:SimpleBigBitmap = null;
		
		private var _canvasTransparent:Boolean = false;
		
		private var _canvasColor:uint = 0x00000000;
		
		private var _mergeAlpha:Boolean = false;
		
		private var _bmpdAreaWidth:uint = 0;
		
		private var _bmpdAreaHeight:uint = 0;
		...
		public function get bitmapDataAreaWidth():uint
		{
			return _bmpdAreaWidth;
		}
		
		public function get bitmapDataAreaHeight():uint
		{
			return _bmpdAreaHeight;
		}
		
		public function clearCanvas():void
		{
			_canvas.fill(_canvasColor);
			DestroyUtil.destroyMap(_cellLoaders);
		}
		
		private function initializeCanvas():void
		{
			_canvas = new SimpleBigBitmap(_bmpdAreaWidth, _bmpdAreaHeight, _mapWidth, _mapHeight, _canvasTransparent, _canvasColor, _mergeAlpha);
			addChild(_canvas);
		}
